        ...
        <v-row>
            <v-col cols="12">
                <v-form v-if="user.status_eva === 2 || user.status_eva === 3">
                    ...
                    <div class="mt-4">
                        <v-card class="pa-2 text-end" color="success">คะแนนรวม : {{ totalScore }} คะแนน</v-card>
                    </div>
                </v-form>
...
const fetchTopic = async () =>{
    ...
    try{
        ...
        res.data.forEach(s => s.indicates.forEach((i) => totalScore.value += (i.score_commit*i.point_indicate)))
    }catch(err){
        console.error('Error GET User!',err)
    }
}
